...
\end{tabular}
\\[0.6em]
{\footnotesize 在 $29$ 段真实轨迹上对\emph{逐段跨运行中位}视界做单边配对 Wilcoxon 符号秩检验
（数据与表体同为 \texttt{data/canon\_multirun/real\_runs.csv}，但聚合次序不同：表体先取每次运行的 $29$ 段中位
再跨运行取中位，配对检验则须先对每段跨运行取中位；两种次序给出的总体倍数一致——
$2.50$/$2.70$ 对 $2.55$/$2.75$），物理先验模型优于 ESN：
PINN $19$/$10$ 段占优（$p = 1.1\times10^{-2}$）、Hybrid $20$/$9$ 段占优（$p = 4.9\times10^{-3}$）。
若改用单次运行口径（\texttt{seed} 42，\texttt{summary\_multi.csv}）则为 PINN $19$/$2$/$8$（$p = 6\times10^{-4}$）、
Hybrid $20$/$2$/$7$（$p = 4\times10^{-4}$）。两者结论方向一致，但显著性水平相差约一个数量级，
故检验结果确实依赖于中位的聚合方式，本文以更保守的跨运行口径为准：
该 \texttt{seed} 的 ESN 段中位为 $0.150$\,s，是 $10$ 个储池种子中最弱的一次运行
（其余为 $0.167$--$0.200$\,s），单取该次会高估物理先验的显著性。}
\end{table}
...
视界随释放角（混沌强度的代理）呈显著下降趋势（\cref{fig:real-horizon}）：逐段取跨运行中位，
视界与释放角的 Spearman 秩相关为 ESN $-0.69$（$p = 4\times10^{-5}$）、PINN $-0.46$（$p = 0.012$）、
Hybrid $-0.45$（$p = 0.014$）；但逐段离散度大，并非逐点单调。

\paragraph{优势随混沌强度而变，弱混沌区物理先验并不占优} 按释放角分档（同为跨运行中位）可以看到
一个必须如实报告的结构：$38^\circ$--$60^\circ$ 档（$n=3$）三者中位为 ESN $\ge 3.99$ / PINN $2.29$ / Hybrid $2.43$\,s，
\emph{物理先验模型反而更差}（$0.58\times$ / $0.61\times$）——该档 $3$ 段中有 $2$ 段的 ESN 在整个 $4$\,s 窗内
始终未超 $10^\circ$ 阈值而被右删失，其中位只是窗长下界、真实值更高（该档仅 $3$ 段，故此处只作定性提示）；
$60^\circ$--$80^\circ$ 档（$n=10$）为 $0.20 / 0.29 / 0.33$\,s（$1.46\times$ / $1.67\times$），
$80^\circ$ 以上档（$n=16$）为 $0.14 / 0.43 / 0.45$\,s（$3.09\times$ / $3.30\times$）。
即物理结构的收益随混沌强度单调增强，而在弱混沌区纯数据方法本就够用甚至更好。
这与 \cref{sec:esn} 的引入窗扫描指向同一件事：物理先验的价值出现在问题最难、可用真实数据最少的一侧；
\cref{tab:real-horizon} 的 $2.5\text{--}2.7$ 倍是 29 段（释放角 $38^\circ$--$111^\circ$）的总体中位，
不应被读成在任意释放角下都成立。
...
而物理模型在此不利口径下总体仍占优，反而强化了结论。但这一优势并非在所有释放角上成立：
弱混沌档（$38^\circ$--$60^\circ$）ESN 反而更好，且该档的右删失几乎全部来自 ESN
（\cref{sec:sim2real-horizon}），我们如实报告分档结果而非只给总体倍数。
...
需说明的是，该核验为求自包含可复现，在建网前显式播种重训了自己的一批运行，
与 \cref{tab:real-horizon} 所用运行并非同一批，故 $10^\circ$ 档的绝对倍数
（Hybrid $2.80$ / PINN $2.30$）与表体（$2.70$ / $2.50$）略有出入；
此处关心的是倍数在换阈值时是否反转，而非其绝对大小。
