...
# 实物是分布质量物理摆：铝臂质量沿杆分布 + 下摆末端配重，
# 故用质心距 (LC) 与转动惯量 (I) 建模，而非把质量当成杆末端一个质点。
# 点质量模型是本式的特例：LC1=L1, LC2=L2, I1_O=M1·L1², I2_A=M2·L2²。
G  = 9.81                          # 重力加速度 m/s²
L1 = 0.250                         # 关节间距 = 上下轴销孔心距 m
_ARM1, _ARM2 = 0.09156, 0.07324    # 上/下铝臂(均匀杆) kg：总臂重 164.8g 按长度 250:200 分
_CW = 0.0639                       # 配重 kg，位于下摆末端 0.20m
...
I2_A = _ARM2 * 0.20**2 / 3 + _CW * 0.20**2 + 0.5 * _CW * 0.015**2   # 下摆对关节
